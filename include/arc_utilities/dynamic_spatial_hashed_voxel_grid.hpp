...
struct CHUNK_REGION
{
  ...
  CHUNK_REGION() : base(Eigen::Vector4d(0.0, 0.0, 0.0, 1.0)) {}

  CHUNK_REGION(const double base_x, const double base_y, const double base_z)
    : base(Eigen::Vector4d(base_x, base_y, base_z, 1.0)) {}

  CHUNK_REGION(const Eigen::Vector4d& in_base) : base(in_base)
  {
    if (base(3) != 1.0)
    {
  ...
  {
    if (EigenHelpers::Equal4d(base, other.base))
    {
      return true;
    }
    else
...
protected:
  ...
  inline int64_t GetLocationDataIndex(const Eigen::Vector4d& location) const
  {
    ...
    if (location_wrt_chunk(0) < 0.0 || location_wrt_chunk(1) < 0.0
        || location_wrt_chunk(2) < 0.0)
    {
      return -1;
    }
    else if (location_wrt_chunk(0) > chunk_x_size_
             || location_wrt_chunk(1) > chunk_y_size_
             || location_wrt_chunk(2) > chunk_z_size_)
    {
      return -1;
    }
    ...
    {
      const int64_t x_cell = (int64_t)(location_wrt_chunk(0) / cell_x_size_);
      const int64_t y_cell = (int64_t)(location_wrt_chunk(1) / cell_y_size_);
      const int64_t z_cell = (int64_t)(location_wrt_chunk(2) / cell_z_size_);
      if (x_cell < 0 || y_cell < 0 || z_cell < 0 || x_cell >= num_x_cells_
          || y_cell >= num_y_cells_ || z_cell >= num_z_cells_)
      {
        return -1;
      }
      else
...
public:
  ...
  DynamicSpatialHashedVoxelGridChunk(const CHUNK_REGION& region,
  ...
  DynamicSpatialHashedVoxelGridChunk(const CHUNK_REGION& region,
                                     const double chunk_x_size,
                                     const double chunk_y_size,
                                     const double chunk_z_size,
                                     const T& initial_value)
  {
  ...
  inline GridQuery<const T> GetImmutableByIndex(const int64_t x_index,
                                                const int64_t y_index,
                                                const int64_t z_index) const
  {
    if (IndexInBounds(x_index, y_index, z_index))
    {
      ...
      return GridQuery<const T>(AccessIndex(data_index));
    }
    else
    {
  ...
  inline GridQuery<T> GetMutableByIndex(const int64_t x_index,
                                        const int64_t y_index,
                                        const int64_t z_index)
  {
    ...
    {
      const int64_t data_index = GetDataIndex(x_index, y_index, z_index);
      return GridQuery<T>(AccessIndex(data_index));
    }
    else
    {
      return GridQuery<T>();
  ...
  inline GridQuery<const T> GetCellImmutable(
      const Eigen::Vector4d& location) const
  {
    if (cell_initialized_)
    {
    ...
    {
      throw std::runtime_error("Chunk is not cell initialized");
    }
  }

  inline GridQuery<T> GetCellMutable(const Eigen::Vector4d& location)
  {
    if (cell_initialized_)
    {
      const int64_t data_index = GetLocationDataIndex(location);
  ...
  inline T& GetChunkMutable()
  {
    ...
    {
      throw std::runtime_error("Chunk is not chunk initialized");
    }
  }
  ...
  inline bool SetCellValue(const Eigen::Vector4d& location, const T& value)
  {
    if (cell_initialized_)
    {
      const int64_t data_index = GetLocationDataIndex(location);
  ...
  inline bool SetCellValue(const Eigen::Vector4d& location, T&& value)
  {
    if (cell_initialized_)
    {
      const int64_t data_index = GetLocationDataIndex(location);
  ...
  inline std::vector<double> GetIndexLocationInGrid(const int64_t x_index,
                                                    const int64_t y_index,
                                                    const int64_t z_index) const
  {
    if (IndexInBounds(x_index, y_index, z_index))
    {
      if (chunk_initialized_)
      {
        const Eigen::Vector4d point_in_chunk_frame(chunk_x_size_ * 0.5,
                                                   chunk_y_size_ * 0.5,
                                                   chunk_z_size_ * 0.5,
                                                   1.0);
        const Eigen::Vector4d point_in_grid_frame
            = region_.base + point_in_chunk_frame;
        return std::vector<double>{point_in_grid_frame(0),
                                   point_in_grid_frame(1),
                                   point_in_grid_frame(2)};
      }
      else if (cell_initialized_)
      {
        const Eigen::Vector4d point_in_chunk_frame(
        ...
        const Eigen::Vector4d point_in_grid_frame
            = region_.base + point_in_chunk_frame;
        return std::vector<double>{point_in_grid_frame.x(),
  ...
  inline std::vector<double> GetElementSize() const
  {
    ...
    {
      throw std::runtime_error("Chunk is uninitialized");
    }
  }
...
protected:
  ...
  T default_value_;
  std::unordered_map<CHUNK_REGION,
                     DynamicSpatialHashedVoxelGridChunk<T, Allocator>> chunks_;
  double chunk_x_size_;
  double chunk_y_size_;
  double chunk_z_size_;
  double cell_x_size_;
  ...
  inline void SafetyCheckSizes(const double cell_x_size,
                               const double cell_y_size,
                               const double cell_z_size,
                               const int64_t chunk_num_x_cells,
                               const int64_t chunk_num_y_cells,
                               const int64_t chunk_num_z_cells) const
  {
    ...
    if (chunk_num_x_cells <= 0)
    {
      throw std::invalid_argument(
            "chunk_num_x_cells must be positive and non-zero");
    }
    if (chunk_num_y_cells <= 0)
    {
      throw std::invalid_argument(
            "chunk_num_y_cells must be positive and non-zero");
    }
    if (chunk_num_z_cells <= 0)
    {
      throw std::invalid_argument(
            "chunk_num_z_cells must be positive and non-zero");
    }
  }
  ...
  inline void CoreInitialize(const double cell_x_size,
                             ...
                             const int64_t chunk_num_x_cells,
                             const int64_t chunk_num_y_cells,
                             const int64_t chunk_num_z_cells,
                             const T& default_value)
  {
    SafetyCheckSizes(cell_x_size, cell_y_size, cell_z_size,
                     chunk_num_x_cells, chunk_num_y_cells, chunk_num_z_cells);
    cell_x_size_ = fabs(cell_x_size);
    cell_y_size_ = fabs(cell_y_size);
    cell_z_size_ = fabs(cell_z_size);
...
public:
  ...
  DynamicSpatialHashedVoxelGrid(const double cell_x_size,
                                const double cell_y_size,
                                const double cell_z_size,
                                const int64_t chunk_num_x_cells,
                                const int64_t chunk_num_y_cells,
  ...
  DynamicSpatialHashedVoxelGrid(const double cell_size,
                                const int64_t chunk_num_x_cells,
                                const int64_t chunk_num_y_cells,
                                const int64_t chunk_num_z_cells,
                                const T& default_value)
  ...
  inline std::pair<const T&, FOUND_STATUS> GetImmutable(const double x,
                                                        const double y,
                                                        const double z) const
  {
    return GetImmutable(Eigen::Vector4d(x, y, z, 1.0));
  }

  inline std::pair<T&, FOUND_STATUS> GetMutable(const double x,
                                                const double y,
                                                const double z)
  {
    return GetMutable(Eigen::Vector4d(x, y, z, 1.0));
  }

  inline SET_STATUS SetCellValue(const double x,
                                 const double y,
                                 const double z,
                                 const T& value)
  {
    return SetCellValue(Eigen::Vector4d(x, y, z, 1.0), value);
  }
  ...
  inline SET_STATUS SetChunkValue(const double x,
                                  const double y,
                                  const double z,
                                  const T& value)
  {
  ...
  inline CHUNK_REGION GetContainingChunkRegion(
      const Eigen::Vector4d& grid_location) const
  {
    if (initialized_)
    {
      ...
      // contains it.
      const double raw_x_chunk_num = grid_location(0) / chunk_x_size_;
      const double raw_y_chunk_num = grid_location(1) / chunk_y_size_;
      const double raw_z_chunk_num = grid_location(2) / chunk_z_size_;
      const int64_t x_chunk_num = (int64_t)std::floor(raw_x_chunk_num);
      const int64_t y_chunk_num = (int64_t)std::floor(raw_y_chunk_num);
      const int64_t z_chunk_num = (int64_t)std::floor(raw_z_chunk_num);
      const double region_base_x = (double)x_chunk_num * chunk_x_size_;
      const double region_base_y = (double)y_chunk_num * chunk_y_size_;
      const double region_base_z = (double)z_chunk_num * chunk_z_size_;
      return CHUNK_REGION(region_base_x, region_base_y, region_base_z);
    }
    else
    {
  ...
  inline std::pair<const T&, FOUND_STATUS> GetImmutable(
      const Eigen::Vector4d& location) const
  {
    if (initialized_)
    {
      ...
      const CHUNK_REGION region = GetContainingChunkRegion(grid_frame_location);
      ...
      if (found_chunk_itr != chunks_.end())
      {
        ...
        {
          // Get the data
          std::pair<const T&, bool> found_in_cell = chunk.GetCellImmutable(grid_frame_location);
  ...
  {
    return std::vector<double>{cell_x_size_ * (double)chunk_num_x_cells_,
                               cell_y_size_ * (double)chunk_num_y_cells_,
  ...
  inline std::vector<int64_t> GetChunkNumCells() const
  {
    return std::vector<int64_t>{
      chunk_num_x_cells_, chunk_num_y_cells_, chunk_num_z_cells_};
  }
  ...
  inline Eigen::Isometry3d GetInverseOriginTransform() const
  {
  ...
  inline const
  std::unordered_map<CHUNK_REGION,
                     DynamicSpatialHashedVoxelGridChunk<T, Allocator>>&
  GetInternalChunks() const
  {
    return chunks_;
  }
};
